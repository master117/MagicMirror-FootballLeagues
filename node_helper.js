...
module.exports = NodeHelper.create( {
        ...
        startGatherData: function (leagues, showLogos, showTables, apiKey, id, showUnavailable) {
            ...
            if (showUnavailable) {
                for (var j = 0; j < leagues.length; j++) {
                    ...
                    fetch('http://api.football-data.org/v2/competitions/' + league.toString(), {
                          options
        ...
        getMatches: function (leagueId, apiKey, refreshTime, id)
        {
            ...
            fetch('http://api.football-data.org/v2/competitions/' + leagueId.toString() + '/matches?dateFrom=' + begin + '&dateTo=' + end, {
                    options
            }).then(function (error, response, body) {
                try {	
        ...
        getTable: function (leagueId, apiKey, refreshTime, id) {
            ...
            fetch('http://api.football-data.org/v2/competitions/' + leagueId.toString() + '/standings', {
                    options
            }).then(function (error, response, body) {
                try {
                    var data = JSON.parse(body);                  
        ...
        getTeamLogos: function (leagueId, apiKey, id) {
            ...
            fetch('http://api.football-data.org/v2/competitions/' + leagueId.toString() + '/teams', {
                    options
            }).then(function (error, response, body) {
                try {
                    var teamLogos = {};
